...
function formatPrivateKey(key: string) {
  // 1. Remove wrapping double quotes if they exist (common .env mistake)
  const cleaned = key.replace(/^"|"$/g, "");
  
  // 2. Handle literal "\n" characters (from JSON) vs actual newlines
  // If the key looks like it has literal "\n", replace them with real newlines
  if (cleaned.includes("\\n")) {
...
export function getAdminApp() {
  ...
  if (!projectId || !clientEmail || !privateKey) {
    // Log a helpful error to the server console
