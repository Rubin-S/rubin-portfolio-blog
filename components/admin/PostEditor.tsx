...
export default function PostEditor({ initialPost, allSeries = [] }: { initialPost: EditorPost; allSeries?: Series[] }) {
  ...
  const publishedAtLabel = useMemo(() => formatTimestamp(initialPost.publishedAt), [initialPost.publishedAt]);
  ...
  const persist = useCallback(
    ...
    [editor, title, excerpt, tagsInput, initialPost.slug, resetFeedback, status, seriesId, seriesIndex]
  );
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-baseline justify-between gap-4 border-b border-zinc-800 pb-4">
        <div>
          ...
          <div className="mt-1 inline-flex items-center gap-2">
            <span
              className={`rounded-full px-3 py-1 text-xs font-semibold ${status === "published"
            ...
            {publishedAtLabel && (
              <span className="text-xs text-zinc-500 dark:text-zinc-400">Published {publishedAtLabel}</span>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="space-y-3">
          ...
          {seriesId && (
            <label className="block space-y-1">
              <span className="text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Series Index</span>
              <input
                type="number"
                value={seriesIndex}
                onChange={(event) => {
        ...
        {editor ? (
          <>
            <div className="mb-2 flex gap-2 border-b border-zinc-800 pb-2">
              <button onClick={addImage} className="text-xs bg-zinc-800 px-2 py-1 rounded hover:bg-zinc-700">Add Image</button>
            </div>
